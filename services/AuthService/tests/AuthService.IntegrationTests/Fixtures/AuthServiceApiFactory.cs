...
public class AuthServiceApiFactory : WebApplicationFactory<Program>
{
    ...
    private readonly string? _seedAdminPassword;

    public AuthServiceApiFactory(string connectionString, string? seedAdminEmail = null, string? seedAdminPassword = null)
    {
        _connectionString = connectionString;
        _seedAdminEmail = seedAdminEmail;
        _seedAdminPassword = seedAdminPassword;
    }
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        if (_seedAdminEmail is not null && _seedAdminPassword is not null)
        {
            builder.ConfigureAppConfiguration(config => config.AddInMemoryCollection(
            [
                new("SeedAdmin:Email", _seedAdminEmail),
                new("SeedAdmin:Password", _seedAdminPassword)
            ]));
        }
