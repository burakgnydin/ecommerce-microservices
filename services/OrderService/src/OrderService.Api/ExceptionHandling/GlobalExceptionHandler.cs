...
public class GlobalExceptionHandler : IExceptionHandler
{
    ...
    public async ValueTask<bool> TryHandleAsync(HttpContext httpContext, Exception exception, CancellationToken cancellationToken)
    {
        var (statusCode, title, detail) = exception switch
        {
            ...
            ProductNotFoundException => (StatusCodes.Status404NotFound, "Product not found", exception.Message),
            InsufficientStockException => (StatusCodes.Status409Conflict, "Insufficient stock", exception.Message),
            InvalidOrderStatusException => (StatusCodes.Status400BadRequest, "Invalid order status transition", exception.Message),
            EmptyCartException => (StatusCodes.Status400BadRequest, "Cart is empty", exception.Message),
            ProductServiceUnavailableException => (StatusCodes.Status503ServiceUnavailable, "Product service unavailable", exception.Message),
